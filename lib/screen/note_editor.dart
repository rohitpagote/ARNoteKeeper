...
import 'package:flutter_unity_example/screen/unity_view.dart';
...
class _NoteEditorState extends State<NoteEditor> with CommandHandler {
  ...
  List<Widget> _buildTopActions(BuildContext context, String uid) => [
        IconButton(
            ...
            icon: Icon(
              AppIcons.brush_sharp,
              color: Colors.white,
            ),
            onPressed: () => showDialog(
                  context: context,
                  builder: (context) => AlertDialog(
                    content: const Text('Title should not be duplicated.'),
                    actions: [
                      FlatButton(
                        child: const Text('OK'),
        ...
        IconButton(
            ...
            onPressed: () => showDialog(
                  context: context,
                  builder: (context) => AlertDialog(
                    content: const Text('Title should not be empty.'),
                    actions: [
                      FlatButton(
                        child: const Text('OK'),
                        onPressed: () => Navigator.pop(context),
        ...
        IconButton(
          ...
          onPressed: () {
            _updateNoteState(
                uid, _note.AR ? NoteState.unspecified : NoteState.AR);
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              IconButton(
                ...
                onPressed: () {
                  Share.share(
                      '${_note.title.trim() != null ? _note.title.trim() : ''}\n(On: ${DateTime.now().toIso8601String().substring(0, 10)})\n\n${_note.content != null ? _note.content : ''}');
                },
              ),
